...
Read this when you: design the compilation into a graph, an MCP layer, or an external consumer; consider introducing a new relation; decompose an interface; or need to know exactly what a downstream tool is allowed to assume.
...
- **One source, one shape.** Cards and registry use the same English frontmatter keys and the same edge-type names. The registry adds graph mechanics (deterministic edge ids, interface decomposition) but invents no new vocabulary. This is why there is no longer a translation map to keep in sync — a class of drift simply does not exist.
...
```json
{
  ...
  "label": "Attraction",
  "status": "accepted",
  "source": "lead-gen lead's decision, 2026-06",
  "owner": "lead-gen lead",
...
- `subtype` carries a concept card's `type` value (`product | service | metric | tool | role | position | …`) when `type` is `concept`; otherwise `null`. It is a narrowing label, not a separate axis — keep it in `attrs`-adjacent thinking, not in traversal logic.
...
- `source`, `owner`, `last-reviewed`, `next-audit` are copied verbatim from the card frontmatter. `last-reviewed` / `next-audit` are what the drift-sweep reads to find stale cards.
- `attrs` is an open bag for type-specific structured fields that are not edges (for example an interface's `outcome` and `quality-criterion`, or a decision's `irreversible` flag). Keep relationships out of `attrs` — relationships are edges.

### Frontmatter keys (the locked set)

Every card carries exactly these frontmatter keys, and the node schema mirrors them: `id`, `type`, `status`, `source`, `owner`, `links`, `last-reviewed`, `next-audit`. `links` is the only one that does not become a node field — it compiles into edges (see [Edge schema](#edge-schema)). Anything beyond this set lives in the card body or, when it is structured and machine-relevant, in `attrs`.
...
Any type outside this table is a validation error at compile time (see [Validation](#validation)). The list is deliberately short; it grows only when you genuinely hit a wall, and then the new relation is added to the table — and to the validator — *first*, before any card uses it. That ordering is what keeps the closed list actually closed.

### Direction is part of the meaning

Edges are directed, and the direction encodes the claim. `source-of-truth` runs *from* the fact *to* where it lives (`metric → tool`), not the other way round — so "where does this number come from?" is a forward traversal, and "what facts does this tool own?" is a reverse traversal. When you author a link, point it the way the table reads; a backwards edge is a silent semantic bug that the validator cannot catch.
...
The compiler must reject a registry that violates the contract. At minimum, `scripts/links_validate.py` checks — and any compiler must check — the following, which map one-to-one to the rules above:
...
- every card has an `id` and a `status` (the minimum-frontmatter rule).

Run it before committing an edit and **show the output** — do not assert "validated" in prose. A green validator is evidence; a claim is not.
...
Project binding: if a module already ships a live `registry/<module>-ontology.json` (as attraction does), compilation must produce a compatible shape — diff your keys and edge types against that file rather than spawning a second, incompatible contract. The whole value of this spec is that there is exactly one shape downstream consumers must learn.
...
type: decision
status: implemented
# attrs: irreversible=false, episode="2026-05 policy call", scope="attraction"
```

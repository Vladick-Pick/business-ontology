...
- **Факты Skribby** (docs.skribby.io, проверено 2026-07-05; конспект —
  ...
  - события на webhook_url: `{"type": "<event>", "data": {...}}`; `GET /bot/{id}` —
    статус/результат; транскрипт со speaker diarization + timestamps;
  - точную схему события «transcript ready» взять из
    docs.skribby.io/rest-api/openapi при написании mapping-а (см. Step 3).
...
4. **Возврат**: `hooks.mappings` превращает событие «transcript ready» в агентный
   ход «process meeting transcript <bot_id>»; агент забирает транскрипт
   (`GET /bot/{id}`), редактирует (PII), оформляет событие-источника
...
- [ ] Тесты OK, евалы 0 failed, ≥3 коммита

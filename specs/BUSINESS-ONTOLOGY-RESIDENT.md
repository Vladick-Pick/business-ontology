...
- **Model health as observability.** A resident runtime SHOULD expose a read-only `modelHealth` projection with accepted/candidate/hypothesis/conflict counts, stale past-next-audit count, owner/source-locator coverage, unanswered competency questions, proposals blocked by missing owner, and high-risk review WIP against a five-item limit. Missing inputs are reported explicitly; the projection never accepts or rejects truth.
